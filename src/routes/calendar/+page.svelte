<script lang="ts">
	...
	const calendarDays = $derived(() => {
		...
		const days: Array<{ date: number | null; dateStr: string; sessions: typeof sessions }> = [];
		...
		for (let d = 1; d <= lastDay.getDate(); d++) {
			...
			days.push({ date: d, dateStr, sessions: daySessions });
		}
	...
	{:else}
		...
		<div class="grid grid-cols-7 gap-1 mb-4">
			{#each calendarDays() as day}
				...
				{:else}
					<button
						onclick={() => selectedDate = day.sessions.length > 0 ? day.dateStr : null}
						class="aspect-square flex flex-col items-center justify-center rounded-lg text-sm transition-colors relative
							...
							{selectedDate === day.dateStr ? 'bg-accent text-white' : day.sessions.length > 0 ? 'bg-dark-card hover:bg-dark-surface' : 'text-text-secondary hover:bg-dark-surface/50'}"
					>
						{day.date}
